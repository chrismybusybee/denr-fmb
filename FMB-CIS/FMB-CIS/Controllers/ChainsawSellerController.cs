    ...
    public class ChainsawSellerController : Controller
    {
        ...
        public IActionResult ChainsawSellerApproval(string uid, string appid)
        {
            ...
            else
            {
                ...
                List<PermitProgressStatusModel> permitPath = _permitStepCountService.MapPermitProgressStatus(permitTypeID, loggedUserRegionId).ToList();
        ...
        public IActionResult ChainsawSellerApplicantsList()
        {
            ...
            int permitToPurchaseId = 2;
            int permitToSellId = 3;

            List<PermitProgressStatusModel> permitToPurchasePath = _permitStepCountService.MapPermitProgressStatus(permitToPurchaseId, loggedUserRegionId);
            var maxPermitToPurchaseStepCount = permitToPurchasePath.Count(p => p.isHappyPath);

            List<PermitProgressStatusModel> permitToSellPath = _permitStepCountService.MapPermitProgressStatus(permitToSellId, loggedUserRegionId);
            var maxPermitToSellStepCount = permitToSellPath.Count(p => p.isHappyPath);

            var permitToPurchaseApplications = (from aMod in applicationMod
                                                join pPath in permitToPurchasePath
                                                on aMod.status.ToString() equals pPath.ApplicationStatusCode into gj
                                                from subPPath in gj.DefaultIfEmpty()
                                                where aMod.permit_type_id == permitToPurchaseId
                                                select new ApplicantListViewModel
                                                {
                                                    ...
                                                    currentMaxCount = subPPath != null ? maxPermitToPurchaseStepCount : null,
                                                    currentPercentage = subPPath != null ? (subPPath.Progress * 100 / maxPermitToPurchaseStepCount) : null,
                                                    isRead = aMod.isRead,
                                                    date_of_expiration = aMod.date_of_expiration,
                                                }).ToList();

            var permitToSellApplications = (from aMod in applicationMod
                                                join pPath in permitToSellPath
                                                on aMod.status.ToString() equals pPath.ApplicationStatusCode into gj
                                                from subPPath in gj.DefaultIfEmpty()
                                                where aMod.permit_type_id == permitToSellId
                                                select new ApplicantListViewModel
                                                {
                                                    ReferenceNo = aMod.ReferenceNo,
                                                    id = aMod.id,
                                                    ...
                                                    date_due_for_officers = aMod.date_due_for_officers,
                                                    currentStepCount = subPPath != null ? subPPath.Progress : null,
                                                    currentMaxCount = subPPath != null ? maxPermitToSellStepCount : null,
                                                    currentPercentage = subPPath != null ? (subPPath.Progress * 100 / maxPermitToSellStepCount) : null,
                                                    isRead = aMod.isRead,
                                                    date_of_expiration = aMod.date_of_expiration,
                                                }).ToList();

            var applicationModWithDynamicSteps = new List<ApplicantListViewModel>(permitToPurchaseApplications.Count +
                                    permitToSellApplications.Count);
            applicationModWithDynamicSteps.AddRange(permitToPurchaseApplications);
            applicationModWithDynamicSteps.AddRange(permitToSellApplications);
